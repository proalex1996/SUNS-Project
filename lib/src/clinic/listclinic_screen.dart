...
class ListClinicScreen extends StatefulWidget {
  @override
  _ListClinicScreenState createState() => _ListClinicScreenState();
}
...
class _ListClinicScreenState extends State<ListClinicScreen> {
  ...
  @override
  void initState() {
    if (locationBloc.state.clinicNew == null)
      locationBloc.dispatch(SelectLocationOfClinicEvent(
          province: this.locationBloc.state.provinceSelected, type: 3));
    super.initState();
  }
  ...
  _getClinic() {
    return BlocProvider<LocationEvent, LocationState, LocationBloc>(
        bloc: locationBloc,
        builder: (state) {
          var checkState = state.clinicNew == null;
          return checkState
              ? Container()
              : Expanded(
                  child: ListView.builder(
                    scrollDirection: Axis.vertical,
                    itemCount: state.clinicNew?.length ?? 0,
                    itemBuilder: (BuildContext context, int index) {
                      var data = state.clinicNew[index];
                      return Padding(
