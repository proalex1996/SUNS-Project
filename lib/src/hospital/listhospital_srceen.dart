...
import 'package:suns_med/src/Widgets/hospital_item/session_hospital_item_bloc.dart';
...
class ListHospitalScreen extends StatefulWidget {
  @override
  _ListHospitalScreenState createState() => _ListHospitalScreenState();
}

class _ListHospitalScreenState extends State<ListHospitalScreen> {
  final locationBloc = LocationBloc();
  final bloc = HospitalItemBloc();
  ProvinceModel _province;
  CompanyType companyType;
  @override
  void initState() {
    // bloc.dispatch(LoadItemEvent(
    //     id: this.locationBloc.state.provinceSelected.id, type: 4));
    if (locationBloc.state.hospitalNew == null)
      locationBloc.dispatch(SelectLocationOfHospitalEvent(
          province: this.locationBloc.state.provinceSelected, type: 4));
    super.initState();
  }
  ...
  _loadHospital() {
    return BlocProvider<LocationEvent, LocationState, LocationBloc>(
        bloc: locationBloc,
        builder: (state) {
          var checkState = state.hospitalNew == null;
          return checkState
              ? Container()
              : Expanded(
                  child: ListView.builder(
                    scrollDirection: Axis.vertical,
                    itemCount: state.hospitalNew?.length ?? 0,
                    itemBuilder: (BuildContext context, int index) {
                      var data = state.hospitalNew[index];
                      return Padding(
