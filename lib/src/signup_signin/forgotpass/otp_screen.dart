...
import 'package:suns_med/src/signup_signin/forgotpass/updatepass_screen.dart';
...
class OTPPasswordScreen extends StatefulWidget {
  ...
  OTPPasswordScreen({this.forgotPassword});
  @override
...
class _OTPPasswordScreenState extends State<OTPPasswordScreen> {
        ...
        child: Column(
          children: <Widget>[
            ...
            Container(
              ...
              child: Text(_checkPhoneNumber(widget.forgotPassword.phoneNumber),
                  style: TextStyle(
                      fontFamily: 'Montserrat-M',
                      fontSize: 24,
  ...
  void _signInWithPhoneNumberAndSMSCode() async {
    try {
      ...
      try {
        ...
        widget.forgotPassword.token = await user.getIdToken();
        Navigator.pushReplacement(
            context,
            MaterialPageRoute(
                builder: (ctx) => UpdatePassScreen(
                    forgotPasswordModel: widget.forgotPassword)));
      } catch (e) {
  ...
  _checkPhoneNumber(String phoneNumber) {
    if (phoneNumber.startsWith("0")) {
      return '+84${widget.forgotPassword.phoneNumber.substring(1)}';
    } else {
      return "+$phoneNumber";
    }
  }
  ...
  _resendToken() async {
    await FirebaseAuth.instance.verifyPhoneNumber(
      phoneNumber: _checkPhoneNumber(widget.forgotPassword.phoneNumber),
  ...
  _verifySUNSOTP() async {
    try {
      ...
      widget.forgotPassword.token = token;
      Navigator.pushReplacement(
          context,
          MaterialPageRoute(
              builder: (ctx) => UpdatePassScreen(
